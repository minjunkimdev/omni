...
lazy_static! {
    ...
    static ref RE_LOG_DATE: Regex = Regex::new(r"(\d{4}-\d{2}-\d{2}|\d{2}/\d{2}/\d{4}|\d{2}/[a-zA-Z]{3}/\d{4})").unwrap();
    static ref RE_LOG_SEV: Regex = Regex::new(r"\[(INFO|ERROR|WARN|WARNING|DEBUG|FATAL)\]").unwrap();
...
pub fn classify(input: &str) -> ContentType {
    ...
    if has_a && has_b { return ContentType::GitDiff; }
    if lines.iter().take(10).any(|l| l.starts_with("@@ -")) { return ContentType::GitDiff; }

    // 2. GitStatus
    let status_kw = ["On branch ", "HEAD detached", "Changes to be committed", "Changes not staged", "nothing to commit, working tree clean"];
    if lines.iter().any(|l| status_kw.iter().any(|k| l.contains(k))) {
    ...
    for l in &lines {
        if RE_GIT_LOG_HASH.is_match(l) { hash_lines += 1; }
    ...
    let test_kw = ["test result: ", "pytest", "Test Suites:", "Tests:", "--- PASS", "--- FAIL", "✓", "✗"];
    ...
    // 4. BuildOutput
    let build_kw = ["error[E", "error:", "Error:", "warning[", "warning:", "Compiling ", "Finished ", "error TS", "npm error", "pip install failed"];
    ...
    // 6. InfraOutput
    let has_kubectl_header = lines.iter().any(|l| l.contains("NAME") && l.contains("READY") && l.contains("STATUS"));
    if has_kubectl_header { return ContentType::InfraOutput; }
    
    let infra_kw = ["aws ", "Terraform will", "Terraform has", "Terraform plan", r#"Step \d+/\d+"#, "Successfully built"];
    ...
    if lines.iter().any(|l| l.starts_with("Step ") && l.contains('/')) {
        return ContentType::InfraOutput;
    }
    ...
    // 8. TabularData
    if lines.len() >= 3 {
        let aligned_lines = lines.iter().filter(|l| RE_TABULAR_SPACES.is_match(l)).count();
        if aligned_lines >= 2 {
...
mod tests {
    ...
    fn test_classify_git_status_dirty() {
        let input = "On branch feature/omni\nChanges not staged for commit:\n  modified:   src/main.rs";
    ...
    fn test_classify_docker_build_output() {
        let docker = "Step 1/5 : FROM alpine:latest\n ---> 49f356fa4eb1\nStep 2/5 : RUN apk add curl";
    ...
    fn bench_classify_1kb() {
        ...
        assert!(per_iter_us < 100, "took {}µs per iter, expected < 100µs", per_iter_us);
    }
    ...
    fn bench_classify_10kb() {
        ...
        assert!(per_iter_us < 1000, "took {}µs per iter, expected < 1000µs", per_iter_us);
    }
}
