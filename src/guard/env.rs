...
pub const DENYLIST: &[&str] = &[
    "BASH_ENV", "ENV", "ZDOTDIR", "BASH_PROFILE",
...
mod tests {
    ...
    fn test_sanitize_env_menghapus_ld_preload() {
        unsafe { std::env::set_var("LD_PRELOAD", "bad.so"); }
    ...
    fn test_sanitize_env_menghapus_semua_denylist_entries() {
        for key in DENYLIST {
            unsafe { std::env::set_var(key, "malicious_payload"); }
        }
    ...
    fn test_sanitize_env_mempertahankan_path_and_normal_vars() {
        ...
        let has_normal = sanitized.iter().any(|(k, v)| k == "NORMAL_VAR" && v == "123");
